...
let coursesList = []; // будет загружаться из БД, если нужно
...
async function loadTrainees() {
    try {
        ...
        renderTrainees();
        updateCounters();
        // Также обновляем отчёты, если они открыты
...
function getTraineeStatus(trainee) {
    // Определяем статус на основе курсов и аттестаций
...
function updateCounters() {
    document.getElementById('traineesCount').innerText = traineesData.length;
    // Активные обучения – количество записей в прохождении_обучения со статусом 'в процессе'
    // Загружаем из API, но для простоты можно посчитать локально
    // В loadStats уже обновляется активные обучения, поэтому здесь просто вызываем loadStats
    loadStats();
}
...
async function saveNewTrainee() {
    ...
    try {
        const response = await fetch('api/add_trainee.php', {
            ...
            body: JSON.stringify({ fullName, startDate, status, notes })
        });
...
async function saveTraineeFromModal() {
    ...
    try {
        const response = await fetch('api/update_trainee.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ id, fullName, startDate })
        });
        const result = await response.json();
        if (result.success) {
            closeModal();
...
async function deleteMentor(id) {
    ...
    try {
        ...
        if (result.success) {
            await loadMentors();
            await loadTrainees(); // обновить таблицу стажёров (у них может измениться mentorId)
        } else {
            alert('Ошибка: ' + result.error);
        }
    } catch (error) {
...
function renderTrainingTable(data) {
    ...
    data.forEach(row => {
        ...
        tr.innerHTML = `
            ...
            <td><progress value="${row.status === 'завершено' ? 100 : 0}" max="100"></progress> ${row.status === 'завершено' ? '100%' : '0%'}</td>
            <td>${currentRole === 'admin' ? '<button class="btn btn-primary btn-sm" onclick="alert(\'Редактирование прогресса\')">Изменить</button>' : ''}</td>
        `;
        tbody.appendChild(tr);
    });
}
...
function renderSettingsCourses() {
    // Этот функционал требует дополнительных API (получение списка курсов, добавление курса)
    // Для простоты оставляем заглушку
    const container = document.getElementById('coursesSettings');
    if (container) {
        container.innerHTML = '<p>Функция управления курсами будет доступна в следующей версии.</p>';
    }
}

function addCourse() {
    alert('Функция добавления курсов будет реализована позже.');
}
